...
export function ConfiguracoesDREDialog({
  ...
  const [showFdcToggle, setShowFdcToggle] = useState(config.incluirFDC || false);

  const diasRestantes = getDiasRestantesForMonth(config);
  const rateioDiarioDespesas = getRateioDiarioDespesasForMonth(config);
  const totalFundoPeriodo = config.metaDiariaFundo * config.totalDiasMes;
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (name) {
      case 'startDate':
        updateDREConfigForMonth(month, year, { startDate: value });
        break;
      case 'endDate':
        ...
        <main className="flex-1 overflow-y-auto p-6 sm:p-10 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex flex-col gap-1.5 md:col-span-2 justify-end">
                <RangeDatePicker
                  ...
                  onChange={(start, end) => {
                    updateDREConfigForMonth(month, year, { startDate: start, endDate: end });
                  }}
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="number"
                  name="metaFundo"
                  value={config.metaDiariaFundo === 0 ? '' : config.metaDiariaFundo}
                  onChange={handleInputChange}
                  step="0.01"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="number"
                  name="percentualCMV"
                  value={config.percentualCMV === 0 ? '' : config.percentualCMV}
                  onChange={handleInputChange}
                  step="0.1"
                  className="bg-slate-900/60 border border-slate-800 text-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-xl p-2 text-sm outline-none"
                />
                ...
                <div className="flex items-center justify-between bg-slate-900/60 border border-slate-800 rounded-xl p-2 h-[38px]">
                  ...
                  <Switch
                    checked={config.prioridadeCMV_DRE || false}
                    onCheckedChange={(checked) => {
                      const updates: Partial<DREConfig> = { prioridadeCMV_DRE: checked };
                      if (!checked) {
                        updates.incluirFDC = false;
                      }
                      updateDREConfigForMonth(month, year, updates);
                    }}
                  />
                </div>
                {config.prioridadeCMV_DRE && (
                  <div className="flex flex-col gap-1">
                    <button
                      onClick={() => setShowFdcToggle(!showFdcToggle)}
                      className="text-[9px] font-bold text-blue-400 hover:text-blue-300 uppercase tracking-wider flex items-center gap-1 cursor-pointer select-none ml-2 w-fit mt-0.5"
                    >
                      incluir FDC {showFdcToggle ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
                    </button>
                    {showFdcToggle && (
                      <div className="flex items-center justify-between bg-slate-900/60 border border-slate-800 rounded-xl p-2 h-[38px] mt-1 animate-in fade-in slide-in-from-top-1 duration-200">
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5 cursor-pointer select-none">
          ...
          <section className="bg-slate-900/40 border border-slate-800/80 p-5 rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-slate-450 uppercase tracking-wider ml-1">Percentual de CMV (%)</label>
                <select
